...
public class DriveOffLine extends SequentialCommandGroup {
  ...
  public DriveOffLine(DriveTrain driveTrain, AdvancedPose2D initialPose, Elevator elevator, CoralAffector coralAffector) {
    ...
    addCommands(new SetInitialPose(driveTrain, initialPose),
                new ParallelCommandGroup(new RawAutonDrive(driveTrain, -.5, 0, 0, 
                                         (FieldConstants.autonLineDistance - AutoAimConstants.blueReef.get(ReefStation.back).getX()) * 2,
                                         0),
                                         new AllToSetPosition(elevator, coralAffector, Position.L4)),
                new AutonCoralScore(coralAffector),
                new RawAutonDrive(driveTrain, .5, 0, 0, 2, 0),
